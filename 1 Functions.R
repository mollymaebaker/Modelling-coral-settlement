...
### Colours representing the attractiveness of different substrates for natural reef substrate configuration from Ningaloo Case Study Site
colstousereal = c("#f5e5d3", "#d4ac7c", "#7bcada", "#1f7aa6", "#552b00")


### Drawing the reef area
...
### Simulating coral larval movement and settlement

### arguments are
### quadname: the name of the reef area (quadrat) to simulate
### hydrodym: name for the hydrodynamic scenario
### rep: rep number
### piceverytime: whether to output a figure every time step
### prsettdef: deafult settlement probabilities for each substrate
### ifreal: whether we are simulating the real reef area
...
### Initialise where larvae will be positioned in the reef area - depending on different hydrodynamic conditions
...
while (t <=ntimesteps & nmove>0){   ## start the main simulation loop
  t=t+1

  #### movement according to hydrodynamic conditions
...
### Oscillating flow
  if (hydrodym=="oscillating"){
    swooshtime = 2 #sec 
    nmove = sum(!larv$ifsett) 
    larv$x[!larv$ifsett] = larv$x[!larv$ifsett]+rnorm(nmove)*turbulence*10*dts + flowrate*dts*sin(t*2*pi/(swooshtime/dts))
    larv$y[!larv$ifsett] = larv$y[!larv$ifsett]+rnorm(nmove)*turbulence*10*dts
    larv$x =   larv$x%%qs[1] 
    larv$y = larv$y%%qs[2]  
  }
...
### Create file name incorporating substrate configuration (quadname), hydrodynamics and replicate names
